(function(){
  ...
  function step(){
    ...
    for (var i = 0; i < nodes.length; i += 1){
      ...
      if (shock && !reduceMotion){
        var sx = n.x - shock.x;
        var sy = (n.y - scrollY) - shock.y;
        var sd = Math.sqrt(sx * sx + sy * sy);
        if (sd < shock.radius && sd > .01){
          var push = (1 - sd / shock.radius) * 8;
          n.vx += sx / sd * push;
          n.vy += sy / sd * push;
        }
      }
    ...
    if (shock){
      ...
      ctx.strokeStyle = 'rgba(37,84,232,' + ((1 - shock.radius / shock.maxR) * .5).toFixed(3) + ')';
      ctx.lineWidth = 2;
      ctx.stroke();
      shock.radius += 14;
      if (shock.radius >= shock.maxR) shock = null;
    }
  ...
  window.addEventListener('mousedown', function(e){
    ...
    shock = { x: e.clientX, y: e.clientY, radius: 18, maxR: 210 };
  });
